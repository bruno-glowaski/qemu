...
static inline void virtio_stw_phys_cached(VirtIODevice *vdev,
                                          MemoryRegionCache *cache, hwaddr pa,
                                          uint16_t value) {
  if (virtio_vdev_is_big_endian(vdev)) {
...
void virtio_init_region_cache(VirtIODevice *vdev, int n) {
  ...
  packed =
      virtio_vdev_has_feature(vq->vdev, VIRTIO_F_RING_PACKED) ? true : false;
  len = address_space_cache_init(&new->desc, vdev->dma_as, addr, size, packed);
  if (len < size) {
  ...
  len = address_space_cache_init(&new->used, vdev->dma_as, vq->vring.used, size,
                                 true);
  if (len < size) {
    g_autofree const char *devname = qdev_get_printable_name(DEVICE(vdev));
  ...
  len = address_space_cache_init(&new->avail, vdev->dma_as, vq->vring.avail,
                                 size, false);
  if (len < size) {
    g_autofree const char *devname = qdev_get_printable_name(DEVICE(vdev));
...
void virtio_queue_update_rings(VirtIODevice *vdev, int n) {
  ...
  vring->used = vring_align(
      vring->avail + offsetof(VRingAvail, ring[vring->num]), vring->align);
...
static void vring_split_desc_read(VirtIODevice *vdev, VRingDesc *desc,
  ...
  address_space_read_cached(cache, i * sizeof(VRingDesc), desc,
                            sizeof(VRingDesc));
...
bool virtio_queue_get_notification(VirtQueue *vq) { return vq->notification; }
...
static void vring_packed_desc_read_flags(VirtIODevice *vdev, uint16_t *flags,
                                         MemoryRegionCache *cache, int i) {
...
static void vring_packed_desc_write_data(VirtIODevice *vdev,
                                         ...
                                         MemoryRegionCache *cache, int i) {
  hwaddr off_id = i * sizeof(VRingPackedDesc) + offsetof(VRingPackedDesc, id);
  hwaddr off_len = i * sizeof(VRingPackedDesc) + offsetof(VRingPackedDesc, len);
...
static bool virtio_queue_packed_poll(VirtQueue *vq, unsigned shadow_idx) {
  ...
  vring_packed_desc_read(vq->vdev, &desc, &caches->desc, shadow_idx, true);
...
bool virtio_queue_enable_notification_and_check(VirtQueue *vq, int opaque) {
...
static void virtqueue_unmap_sg(VirtQueue *vq, const VirtQueueElement *elem,
  ...
  for (i = 0; i < elem->out_num; i++)
    dma_memory_unmap(dma_as, elem->out_sg[i].iov_base, elem->out_sg[i].iov_len,
                     DMA_DIRECTION_TO_DEVICE, elem->out_sg[i].iov_len);
}
...
static void virtqueue_packed_fill_desc(VirtQueue *vq,
                                       ...
                                       unsigned int idx, bool strict_order) {
  uint16_t head;
  VRingMemoryRegionCaches *caches;
  VRingPackedDesc desc = {
      .id = elem->index,
...
static int virtqueue_num_heads(VirtQueue *vq, unsigned int idx) {
  ...
  if (num_heads > vq->vring.num) {
    virtio_error(vq->vdev, "Guest moved used index from %u to %u", idx,
                 vq->shadow_avail_idx);
    return -EINVAL;
  }
  /*
...
static int virtqueue_packed_read_next_desc(VirtQueue *vq, VRingPackedDesc *desc,
                                           MemoryRegionCache *desc_cache,
...
int virtqueue_get_avail_bytes(VirtQueue *vq, unsigned int *in_bytes,
  ...
  if (virtio_vdev_has_feature(vq->vdev, VIRTIO_F_RING_PACKED)) {
    virtqueue_packed_get_avail_bytes(vq, in_bytes, out_bytes, max_in_bytes,
                                     max_out_bytes, caches);
  } else {
    virtqueue_split_get_avail_bytes(vq, in_bytes, out_bytes, max_in_bytes,
                                    max_out_bytes, caches);
  }
 ...
 * yet.
 */
static void virtqueue_undo_map_desc(AddressSpace *as, unsigned int out_num,
                                    unsigned int in_num, struct iovec *iov) {
  unsigned int i;
...
void virtqueue_map(VirtIODevice *vdev, VirtQueueElement *elem) {
  ...
  virtqueue_map_iovec(vdev, elem->out_sg, elem->out_addr, elem->out_num, false);
}
...
static void *virtqueue_split_pop(VirtQueue *vq, size_t sz) {
  ...
  do {
    ...
    if (desc.flags & VRING_DESC_F_WRITE) {
      ...
      map_ok = virtqueue_map_desc(vdev, &out_num, addr, iov, VIRTQUEUE_MAX_SIZE,
                                  false, desc.addr, desc.len);
    }
    if (!map_ok) {
      goto err_undo_map;
    }
...
static unsigned int virtqueue_packed_drop_all(VirtQueue *vq) {
  ...
  while (vq->inuse < vq->vring.num) {
     ...
     */
    vring_packed_desc_read(vdev, &desc, desc_cache, vq->last_avail_idx, true);
    if (!is_desc_avail(desc.flags, vq->last_avail_wrap_counter)) {
      break;
    }
    elem.index = desc.id;
    elem.ndescs = 1;
    while (virtqueue_packed_read_next_desc(vq, &desc, desc_cache, vq->vring.num,
                                           &idx, false)) {
      ++elem.ndescs;
    }
    /*
...
void virtio_queue_set_shadow_avail_idx(VirtQueue *vq,
                                       uint16_t shadow_avail_idx) {
  if (!vq->vring.desc) {
    return;
  }
...
uint16_t virtio_queue_vector(VirtIODevice *vdev, int n) {
  return n < VIRTIO_QUEUE_MAX ? vdev->vq[n].vector : VIRTIO_NO_VECTOR;
}
...
void virtio_queue_set_vector(VirtIODevice *vdev, int n, uint16_t vector) {
  ...
  if (n < VIRTIO_QUEUE_MAX) {
    if (vdev->vector_queues && vdev->vq[n].vector != VIRTIO_NO_VECTOR) {
      QLIST_REMOVE(vq, node);
    }
    vdev->vq[n].vector = vector;
    if (vdev->vector_queues && vector != VIRTIO_NO_VECTOR) {
...
static bool virtio_split_should_notify(VirtIODevice *vdev, VirtQueue *vq) {
  ...
  if (virtio_vdev_has_feature(vdev, VIRTIO_F_NOTIFY_ON_EMPTY) && !vq->inuse &&
...
static bool vring_packed_need_event(VirtQueue *vq, bool wrap, uint16_t off_wrap,
...
static bool virtio_packed_should_notify(VirtIODevice *vdev, VirtQueue *vq) {
  ...
  return !v || vring_packed_need_event(vq, vq->used_wrap_counter, e.off_wrap,
                                       new, old);
}
...
static bool virtio_extra_state_needed(void *opaque) {
  ...
  return k->has_extra_state && k->has_extra_state(qbus->parent);
}
...
static const VMStateDescription vmstate_virtqueue = {
    ...
    .fields = (const VMStateField[]){
        ...
        VMSTATE_UINT64(vring.used, struct VirtQueue), VMSTATE_END_OF_LIST()}};
...
static const VMStateDescription vmstate_packed_virtqueue = {
    ...
    .fields = (const VMStateField[]){
        ...
        VMSTATE_UINT32(inuse, struct VirtQueue), VMSTATE_END_OF_LIST()}};
...
static const VMStateDescription vmstate_virtio_virtqueues = {
    ...
    .fields =
        (const VMStateField[]){VMSTATE_STRUCT_VARRAY_POINTER_KNOWN(
                                   vq, struct VirtIODevice, VIRTIO_QUEUE_MAX, 0,
                                   vmstate_virtqueue, VirtQueue),
                               VMSTATE_END_OF_LIST()}};
...
static const VMStateDescription vmstate_virtio_packed_virtqueues = {
    ...
    .fields =
        (const VMStateField[]){VMSTATE_STRUCT_VARRAY_POINTER_KNOWN(
                                   vq, struct VirtIODevice, VIRTIO_QUEUE_MAX, 0,
                                   vmstate_packed_virtqueue, VirtQueue),
                               VMSTATE_END_OF_LIST()}};
...
static const VMStateDescription vmstate_virtio_device_endian = {
    ...
    .fields = (const VMStateField[]){VMSTATE_UINT8(device_endian, VirtIODevice),
                                     VMSTATE_END_OF_LIST()}};
...
static const VMStateDescription vmstate_virtio_64bit_features = {
    ...
    .fields = (const VMStateField[]){
        VMSTATE_UINT64(guest_features, VirtIODevice), VMSTATE_END_OF_LIST()}};
...
static const VMStateDescription vmstate_virtio_broken = {
    ...
    .fields = (const VMStateField[]){VMSTATE_BOOL(broken, VirtIODevice),
                                     VMSTATE_END_OF_LIST()}};
...
static const VMStateDescription vmstate_virtio_started = {
    ...
    .fields = (const VMStateField[]){VMSTATE_BOOL(started, VirtIODevice),
                                     VMSTATE_END_OF_LIST()}};
...
static const VMStateDescription vmstate_virtio_disabled = {
    ...
    .fields = (const VMStateField[]){VMSTATE_BOOL(disabled, VirtIODevice),
                                     VMSTATE_END_OF_LIST()}};
...
static const VMStateDescription vmstate_virtio = {
    ...
    .subsections = (const VMStateDescription *const[]){
        &vmstate_virtio_device_endian, &vmstate_virtio_128bit_features,
        &vmstate_virtio_64bit_features, &vmstate_virtio_virtqueues,
        &vmstate_virtio_ringsize, &vmstate_virtio_broken,
        &vmstate_virtio_extra_state, &vmstate_virtio_started,
        &vmstate_virtio_packed_virtqueues, &vmstate_virtio_disabled, NULL}};
...
static int coroutine_mixed_fn virtio_device_get(QEMUFile *f, void *opaque,
                                                size_t size,
                                                const VMStateField *field) {
...
virtio_set_features_nocheck_maybe_co(VirtIODevice *vdev, const uint64_t *val) {
...
int coroutine_mixed_fn virtio_load(VirtIODevice *vdev, QEMUFile *f,
                                   int version_id) {
  int i, ret;
  int32_t config_len;
  uint32_t num;
   ...
   */
  if (virtio_set_features_nocheck_maybe_co(vdev, vdev->guest_features_ex) < 0) {
    ...
    if (vdev->vq[i].vring.desc) {
      ...
      if (nheads > vdev->vq[i].vring.num) {
        virtio_error(vdev,
                     ...
                     i, vdev->vq[i].vring.num, vring_avail_idx(&vdev->vq[i]),
       ...
       */
      vdev->vq[i].inuse =
          (uint16_t)(vdev->vq[i].last_avail_idx - vdev->vq[i].used_idx);
...
void virtio_instance_init_common(Object *proxy_obj, void *data,
  ...
  object_initialize_child_with_props(proxy_obj, "virtio-backend", vdev,
                                     vdev_size, vdev_name, &error_abort, NULL);
...
void virtio_init(VirtIODevice *vdev, uint16_t device_id, size_t config_size) {
  ...
  vdev->vmstate = qdev_add_vm_change_state_handler(DEVICE(vdev), NULL,
...
hwaddr virtio_queue_get_avail_size(VirtIODevice *vdev, int n) {
  ...
  return offsetof(VRingAvail, ring) + sizeof(uint16_t) * vdev->vq[n].vring.num +
         s;
}
...
static void virtio_queue_packed_set_last_avail_idx(VirtIODevice *vdev, int n,
                                                   unsigned int idx) {
  struct VirtQueue *vq = &vdev->vq[n];

  vq->last_avail_idx = vq->shadow_avail_idx = idx & 0x7fff;
  vq->last_avail_wrap_counter = vq->shadow_avail_wrap_counter =
      !!(idx & 0x8000);
  idx >>= 16;
...
static void virtio_queue_split_set_last_avail_idx(VirtIODevice *vdev, int n,
                                                  unsigned int idx) {
...
VirtQueue *virtio_get_queue(VirtIODevice *vdev, int n) { return vdev->vq + n; }

uint16_t virtio_get_queue_index(VirtQueue *vq) { return vq->queue_index; }
 ...
 */
void virtio_queue_aio_attach_host_notifier_no_poll(VirtQueue *vq,
                                                   AioContext *ctx) {
  ...
  aio_set_event_notifier(ctx, &vq->host_notifier,
                         virtio_queue_host_notifier_read, NULL, NULL);
...
VirtQueueStatus *qmp_x_query_virtio_queue_status(const char *path,
  ...
  if (vdev->vhost_started) {
    ...
    if (queue >= hdev->vq_index && queue < hdev->vq_index + hdev->nvqs) {
      ...
      int vhost_vq_index = hdev->vhost_ops->vhost_get_vq_index(hdev, queue);
...
VirtioQueueElement *
qmp_x_query_virtio_queue_element(const char *path, uint16_t queue,
                                 bool has_index, uint16_t index, Error **errp) {
  VirtIODevice *vdev;
  VirtQueue *vq;
...
QEMUBH *virtio_bh_io_new_guarded_full(DeviceState *dev, QEMUBHFunc *cb,
                                      void *opaque, const char *name) {
  DeviceState *transport = qdev_get_parent_bus(dev)->parent;
